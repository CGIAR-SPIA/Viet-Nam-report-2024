...
required_packages <- c("sf", "tidyverse", "haven", "foreign", "readxl", 
                       "readr", "curl", "httr", "jsonlite", "maditr", "gridExtra")
...
dat <- read.csv(curl("https://raw.githubusercontent.com/CGIAR-SPIA/Viet-Nam-report-2024/main/data/processed/Rice.vars.VH24.csv"))
...
Years <- read.csv(curl("https://raw.githubusercontent.com/CGIAR-SPIA/Viet-Nam-report-2024/main/data/processed/Rice_Years.csv"))
...
```{r Figure15.wrap, echo=FALSE,fig.asp = 0.8, fig.width = 9, out.width = "100%"}

# Three rice map together
library (gridExtra)
grid.arrange(M.CG[[1]], M.CG[[2]], M.CG[[3]], ncol = 3)

# Convert to a grob object
g <- arrangeGrob(
  M.CG[[1]],
  M.CG[[2]],
  M.CG[[3]],
...
# Save as PNG (or PDF, TIFF, etc.)
ggsave(
  filename = file.path(output_dir, "Fig15.png"),
  plot = g,
  width = 12,   # increase for better resolution
  height = 4,
  dpi = 600
)
#names(M.CG)
#M.CG[[4,5,6,12,15,16,18,19,20]]

#suppress_legend <- function(plot) {plot + theme(legend.position = "none")}

#selected_maps_N <- lapply(M.CG[c(5, 12, 18, 20)], suppress_legend) # Northern
#selected_maps_S <- lapply(M.CG[c(6,15,16)], suppress_legend) # Southern

#grid.arrange(grobs = selected_maps_N, nrow=1, ncol=4) 
#grid.arrange(grobs = selected_maps_S, nrow=1, ncol=3)  

# Issue - legend + breakdown by region is likely more informative? 
...
df_23 <- read.csv(curl("https://raw.githubusercontent.com/CGIAR-SPIA/Viet-Nam-report-2024/main/data/processed/VH23_data.csv"))
...
df_22 <-read.csv(curl("https://raw.githubusercontent.com/CGIAR-SPIA/Viet-Nam-report-2024/main/data/processed/VH22_data.csv"))
Provinces_IDs <- read.csv(curl("https://raw.githubusercontent.com/CGIAR-SPIA/Vietnam-pre-report-2023/main/datasets/Provinces_IDs.csv"))
...
Saltol.map <- modified_map %>%
  ...
  scale_fill_gradient2(midpoint = 50, low = "#f7efef", mid = "#9ecae1", high = "#1c4c6f", 
                       na.value = "#dbd0d0", space = "Lab", name = "% Adopters", limits = c(0, 100)) +  
  theme(plot.title = element_text(size = 8, hjust = 0.5)) +
  geom_rect(aes(xmin = 107.5, xmax = 110, ymin = 8.5, ymax = 10), 
            linewidth = 0.1, color = "black", fill = NA) +
  geom_rect(aes(xmin = 109.3, xmax = 110, ymin = 16.1, ymax = 16.7), 
...
ggsave(Saltol.map, filename = file.path(output_dir, "Fig17.spatial.png"), dpi = 1024)
```
...
df <- read.csv(curl("https://raw.githubusercontent.com/CGIAR-SPIA/Viet-Nam-report-2024/main/data/processed/VH23_data.csv"))
Provinces_IDs <- read.csv(curl("https://raw.githubusercontent.com/CGIAR-SPIA/Vietnam-pre-report-2023/main/datasets/Provinces_IDs.csv"))
...
ggsave(a, filename = file.path(output_dir, "Fig35.spatial.png"), dpi = 1024)
...
download.file("https://raw.githubusercontent.com/CGIAR-SPIA/Viet-Nam-report-2024/main/data/processed/ag_plan_1m5r_recode.xlsx", 
              destfile = temp_file, 
              mode = "wb")  # 'wb' = write binary, required for Excel files
ag_plan <- read_excel(temp_file)
...
create_plot <- function(sf_data, fill_var, plot_title) {
  ggplot(sf_data) +
    geom_sf(aes(fill = factor(.data[[fill_var]], levels = c(1,0))), 
            color = "lightgrey", size = 0.15) +
    scale_fill_manual(
      values = c("1" = "#14456c", "0" = "white"),
      labels = c("Yes", "No"),
      name = NULL,
      drop = FALSE
    ) +
    ...
    theme(
      ...
      panel.border = element_rect(color = "#abcdc7", fill = NA, size = 0.8)
    ) +
...
# Arrange plots in the requested sequence, filling down columns
ordered_plots <- list(plots[[1]], plots[[3]], plots[[5]], plots[[7]],
                      plots[[2]], plots[[4]], plots[[6]], plots[[8]])

# Use wrap_plots to stack them into two columns (down columns)
final_plot <- wrap_plots(ordered_plots, ncol = 4)

ggsave(plot = final_plot, filename = file.path(output_dir, "Fig33.spatial.png"), 
       width = 15, height = 6, dpi = 1024)
